...
Since then, the core realtime slice has landed too: the account page now reflects live `users` changes from Supabase Realtime, the main dashboard request list/count updates live from `requests` inserts, `/api/stream/[slug]` now streams Supabase Realtime events instead of Convex subscriptions, and the guest `/go` live dashboard no longer uses Convex hooks.
...
The next active gaps are the leftover account/API-key UI cleanup, the remaining Playwright verification and cron cleanup jobs, and then the receiver rewrite that finally removes Redis/ClickHouse/Convex from the hot path.
